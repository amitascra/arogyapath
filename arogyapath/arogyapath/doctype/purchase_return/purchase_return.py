...
class PurchaseReturn(Document):
	...
	def validate_return_against(self):
		...
		if not frappe.db.exists("Purchase Invoice", self.return_against):
			frappe.throw(_("Purchase Invoice {0} does not exist").format(self.return_against))
		
		# Fetch invoice details
		invoice = frappe.get_doc("Purchase Invoice", self.return_against)
		
		# Set supplier and branch
		self.supplier = invoice.supplier
		self.supplier_name = invoice.supplier_name
	...
	def set_outstanding_amount(self):
		"""Set outstanding amount for adjustment tracking"""
		if self.docstatus == 0:
			# Draft - outstanding = grand total
			self.outstanding_amount = self.grand_total
			self.return_status = "Pending"
		elif self.docstatus == 1:
			# Submitted - calculate from adjustments
			adjusted_amount = self.get_total_adjusted_amount()
			self.outstanding_amount = flt(self.grand_total) - flt(adjusted_amount)
			
			# Update return status
			if self.outstanding_amount <= 0:
				self.return_status = "Adjusted"
			elif adjusted_amount > 0:
				self.return_status = "Partially Adjusted"
			else:
				self.return_status = "Pending"
	...
	def get_total_adjusted_amount(self):
		"""Get total amount adjusted via Payment Entry"""
		adjusted_amount = frappe.db.sql("""
			SELECT SUM(per.allocated_amount)
			FROM `tabPayment Entry Reference` per
			INNER JOIN `tabPayment Entry` pe ON per.parent = pe.name
			WHERE per.reference_doctype = 'Purchase Return'
				AND per.reference_name = %s
				AND pe.docstatus = 1
		""", self.name)
		
		return flt(adjusted_amount[0][0]) if adjusted_amount else 0
	...
	def update_invoice_outstanding(self):
		...
		try:
			...
			frappe.logger().info(
				f"Purchase Return {self.name}: Reduced outstanding for Purchase Invoice {self.return_against}"
			)
		
		except Exception as e:
			frappe.log_error(
				message=frappe.get_traceback(),
				title=f"Purchase Return Outstanding Update Failed: {self.name}"
			)
			frappe.throw(
				_("Failed to update outstanding for Purchase Invoice {0}. Error: {1}").format(
	...
	def reverse_invoice_outstanding(self):
		...
		try:
			...
			frappe.logger().info(
				f"Purchase Return {self.name}: Reversed outstanding for Purchase Invoice {self.return_against}"
			)
		
		except Exception as e:
			frappe.log_error(
				message=frappe.get_traceback(),
				title=f"Purchase Return Reversal Failed: {self.name}"
			)
